...
hl.config({
    scrolling = {
        ...
        direction = "right", -- horizontal tape; overridden to "down" on workspace 6 (DualUp)
    },
})
...
-- (generic dispatcher: works spatially on both monitors regardless of
-- each workspace's scrolling "direction" - no per-monitor variants needed)
...
-- Swap the focused window with its neighbor (generic, any layout)
...
---- SCROLLING LAYOUT ----
-------------------------------
-- Scroll the view by one column/row without moving focus
...
-- Resize the active column/row through the preconfigured widths
hl.bind(mainMod .. " + equal", hl.dsp.layout("colresize +conf"))
hl.bind(mainMod .. " + minus", hl.dsp.layout("colresize -conf"))

-- Pop the active window into its own column/row, or fold it back in
hl.bind(mainMod .. " + O", hl.dsp.layout("expel"))
hl.bind(mainMod .. " + I", hl.dsp.layout("consume"))

-- Snap the active column/row fully into view
hl.bind(mainMod .. " + SHIFT + F", hl.dsp.layout("fit_into_view"))
